...
def main():
    ...
    parser.add_argument("--epochs", type=int, default=15, help="Number of training epochs per iteration")
    ...
    logger.log_task("Ingesting and Cleaning Dataset", "Executing prepare_dataset.py to map raw files to the 5 active classes...")
    ...
    # LRs to try across loop iterations
    ...
    for iteration in range(1, args.max_iterations + 1):
        ...
        # 1. Train Model
        logger.log_task(f"Model Training (Iteration {iteration})", f"Training CRNN model for {args.epochs} epochs with LR={lr}...")
        t0 = time.time()
        ...
        results_path = os.path.join("checkpoints", "training_results.pth")
        if os.path.exists(results_path):
            checkpoint = torch.load(results_path, map_location="cpu")
            metrics = checkpoint.get("metrics", {})
            train_loss = metrics.get("train_loss", [-1])[-1]
            val_loss = metrics.get("val_loss", [-1])[-1]
            val_acc = metrics.get("val_acc", [-1])[-1]
        else:
            train_loss, val_loss, val_acc = -1, -1, -1
    ...
    if success_iteration != -1:
        ...
        # Add files to git
        run_command(["git", "add", "checkpoints/best_model.pth", "checkpoints/training_results.pth", "../reports/evaluation_report.txt", "../reports/training_run_trace.md"])
